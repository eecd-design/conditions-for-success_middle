...
for (let indicator of allIndicators) {
	let indicatorData = {
		...
		titleWords: indicator.data.title
			? indicator.data.title.toLowerCase().trim().split(' ')
			: [],
...
for (let component of allComponents) {
	let componentData = {
		...
		titleWords: component.data.title
			? component.data.title.toLowerCase().trim().split(' ')
			: [],
	...
	for (let phase of phases) {
		for (let consideration of component.data[phase].considerations) {
			let considerationData = {
				...
				titleWords: consideration.title
					? consideration.title.toLowerCase().trim().split(' ')
					: [],
...
for (let resource of allResources) {
	let resourceData = {
		...
		description: resource.data.description
			...
			: '',
		descWords: resource.data.description
			? resource.data.description.toLowerCase().trim().split(' ')
			: [],
...
for (let training of allTraining) {
	let trainingData = {
		id: `tra-${itemIndex}`,
		category: 'training',
		title: training.data.title.toLowerCase().trim(),
		titleWords: training.data.title ? training.data.title.toLowerCase().trim().split(' ') : [],
		description: training.data.description
			? training.data.description.toLowerCase().trim()
			: '',
		descWords: training.data.description
			? training.data.description.toLowerCase().trim().split(' ')
			: [],
		date: new Date(training.data.dateAdded).getTime(),
		type: training.data.type,
	};
	searchIndex.push(trainingData);
	itemIndex += 1;
}
